...
from triage_poc.sft_termination import audit_labels, native_eos_template, render_training_text

TRAIN_SHA256 = "854ce4f0458d1d313e77d7e3ff9da2d2dffb6571af728a7237f8aada7ce5525d"
...
def main():
    ...
    if sha256(args.train) != TRAIN_SHA256:
    ...
    if len(train) != 4000:
        raise ValueError("Expected 4000 source training rows")
    ...
    if args.dry_run:
        print(json.dumps({"status": "preflight_passed", "arm": args.arm,
                          "train": 64, "validation": 3, "test_records_used": 0}))
        return
    ...
    def restore_model(adapter_directory):
        base, _ = FastLanguageModel.from_pretrained(model_name=BASE_MODEL,
            revision=BASE_REVISION, max_seq_length=2048, dtype=None, load_in_4bit=True,
            use_exact_model_name=True, device_map={"": 0})
        config = json.loads((adapter_directory / "adapter_config.json").read_text())
        restored = FastLanguageModel.get_peft_model(base, r=config["r"],
            lora_alpha=config["lora_alpha"], lora_dropout=config["lora_dropout"],
            target_modules=config["target_modules"], bias="none",
            use_gradient_checkpointing="unsloth", random_state=42, use_rslora=False)
    ...
    marker_probe = {"start_end_embeddings_equal": bool(torch.equal(
    ...
    def generate(stage):
        ...
        for row in validation:
            ...
            reference_ids = tokenizer.encode(render_training_text(reference_tokenizer, row, True),
                                              add_special_tokens=False)
            assert reference_ids[-1] == eos
            with torch.inference_mode():
                logits = model(input_ids=torch.tensor([reference_ids[:-1]],
                ...
                output = model.generate(input_ids=inputs,
                    ...
                    do_sample=False, eos_token_id=[eos, end],
                    pad_token_id=tokenizer.pad_token_id)
            output_ids = output[0, boundary:].tolist()
            records.append({"record_id": row["record_id"], "stage": stage,
        ...
        (args.output / f"{stage}.jsonl").write_text("".join(
            json.dumps(r, ensure_ascii=False) + "\n" for r in records))
        print(json.dumps({"stage": stage, "generated_tokens": [
            r["generated_tokens"] for r in records]}), flush=True)
        return records
    ...
    training_args = SFTConfig(output_dir=str(args.output / "trainer"),
        max_steps=20, per_device_train_batch_size=1, gradient_accumulation_steps=8,
        learning_rate=1e-4, warmup_steps=3, lr_scheduler_type="linear",
        optim="adamw_torch", weight_decay=0.001, fp16=True, bf16=False,
        logging_steps=1, eval_strategy="no", save_strategy="no", report_to="none",
        seed=42, data_seed=42, max_length=2048, packing=False,
        completion_only_loss=False, dataset_text_field="text", dataset_num_proc=1)
    trainer = SFTTrainer(model=model, processing_class=tokenizer,
        train_dataset=Dataset.from_dict({"text": texts}), args=training_args)
    audits = []
    for i in range(len(trainer.train_dataset)):
        batch = trainer.data_collator([trainer.train_dataset[i]])
        audits.append(audit_labels(batch, eos, end))
    ...
    if args.audit_only:
        summary = {"status": "actual_trainer_labels_audited", "arm": args.arm,
            "training_steps": 0, "train_records": len(train), "test_records_used": 0,
            "initial_sft_sha256": SFT_SHA256, "train_sha256": TRAIN_SHA256,
            "native_eos_label_counts": [a["supervised_native_eos"] for a in audits],
            "marker_probe": marker_probe, "code_sha256": sha256(Path(__file__))}
    ...
    reloaded_tokenizer = AutoTokenizer.from_pretrained(str(args.output / "adapter"),
                                                       local_files_only=True)
    for row in train + validation:
        actual = reloaded_tokenizer.apply_chat_template(row["messages"], tokenize=False,
    ...
    changed_tensors = sum(not torch.equal(initial_weights[key], saved_weights[key])
    ...
    summary = {"status": "completed", "purpose": "controlled_sft_termination_experiment",
        ...
        "initial_sft_sha256": SFT_SHA256, "train_sha256": TRAIN_SHA256,
        "validation_sha256": sha256(args.validation), "seed": 42,
        "train_ids": [r["record_id"] for r in train],
        "validation_ids": [r["record_id"] for r in validation],
        "training_args": training_args.to_dict(), "train_metrics": result.metrics,
        "marker_probe": marker_probe, "test_records_used": 0,
        ...
        "package_versions": {p: importlib.metadata.version(p) for p in [
            "unsloth", "unsloth_zoo", "torch", "transformers", "trl", "peft"]},
        "limits": ["64 training rows and three development validations only.",
