...
| GitHub Actions tests/déploiement | 235 tests ; image et factory privée vérifiées localement | Exécution GitHub distante vérifiée et déploiement automatisé |
...
La régression locale complète passe 235 tests au 16 septembre. La v37 a exécuté Base/SFT/DPO via la même API et deux dialogues de six échanges FR/EN. Les garde-fous empêchent les mesures critiques manquantes et les retards dangereux observés, mais interviennent sur 13/18 sorties SFT et 14/18 sorties DPO ; ils ne transforment pas ces mesures en validation clinique. L'image API se construit localement et les modes sans fournisseur et factory privée authentifiée passent hors réseau. La configuration GitHub Actions couvre tests et conteneur ; le déploiement automatisé et son exécution distante restent à terminer. L'unique autorisation GPU actuelle est le notebook Kaggle privé sur quota gratuit. L’ADR-018 propose Modal avec extinction automatique, ou RunPod/Hugging Face en alternatives, mais un endpoint cloud nécessite encore un fournisseur et un plafond de coût explicitement autorisés. Aucun endpoint ni modèle public n'est annoncé.
