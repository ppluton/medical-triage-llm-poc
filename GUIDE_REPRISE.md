...
**État au 16 septembre :** inventaire et intégrité du candidat SFT v2.1 repris dans la [fiche active](docs/technical/CORPUS_ETAPE_1_V1.md) et la [preuve datée](docs/evidence/AUDIT_CORPUS_ETAPE_1_2026-09-16.md). Le schéma de métadonnées et le processus RGPD sont documentés. Le [scan PII contextuel](docs/evidence/SFT_CONTEXTUAL_PII_SCAN_2026-09-16.md) couvre maintenant les 4 700 lignes : 22 lignes ont une alerte directe prioritaire et 3 180 une alerte contextuelle encore à décider. La [consolidation DPO v2](docs/evidence/DPO_PROJECT_REVIEW_V2_2026-09-16.md) est terminée sans modifier les textes d'entraînement. Étape partiellement terminée : les décisions de revue PII et le manifeste final restent ouverts. L'absence d'un référent clinique réel est documentée comme limite de transposition, pas comme blocage de la mission.
...
**État au 16 septembre :** la [revue de sûreté v36](docs/evidence/STAGE2_SAFETY_V36_RESULT_2026-09-16.md) compare Base, SFT et DPO sur les mêmes 18 scénarios avec une file aveugle et des portes proposées dans l'[ADR-016](docs/decisions/ADR-016-portes-surete-etape-2.md). Les trois variantes échouent : les sorties valides au niveau du schéma contiennent encore des affirmations non étayées, des sous-triages et des corruptions de texte. Les [garde-fous déterministes v1](docs/technical/GARDE_FOUS_DETERMINISTES_V1.md) sont implémentés dans l'API 0.4.0. La [v37 réelle](docs/evidence/VLLM_V37_RESULT_2026-09-16.md) restaure les mesures critiques proposées sur les réponses obtenues, mais conserve seulement 4 à 5 sorties sur 18 sans intervention et refuse une génération Base trop longue. Le lot reste un jeu de développement déjà vu ; une nouvelle revue qualitative aveugle manque. Aucun nouveau SFT/DPO n'est décidé à ce stade.
...
**Prochaine action concrète : valider une v38 d'infrastructure utilisant le [snapshot Qwen3 privé](docs/evidence/QWEN3_BASE_KAGGLE_RESOURCE_2026-09-16.md), puis optimiser l'installation vLLM/API sans changer les poids ni les scénarios.** La décision de refaire un SFT ou un DPO viendra seulement après la revue des erreurs de contenu qui subsistent.
